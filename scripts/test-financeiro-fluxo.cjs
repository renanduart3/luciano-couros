...
try {
 ...
 assert.equal(vale().financeiro.recebido,0);assert.equal(vale().financeiro.presumido,900);assert.equal(vale().financeiro.restantePresumido,0);assert.equal(ordem().restantePresumido,0);
 assert.equal(relatorio({statusVenda:'pendente'}).vendas.length,0);assert.equal(relatorio({statusVenda:'paga'}).vendas.length,1);
 let r=relatorio({valeStatus:'quitados'});assert.equal(r.vales.length,1);assert.equal(r.vales[0].saldoRestante,0);assert.equal(r.pagamentos.length,1);assert.equal(r.clientesResumo.find(c=>c.clienteId==='c').saldoDevedor,0);
 ...
 assert.equal(vale().financeiro.recebido,0);assert.equal(vale().financeiro.presumido,900);assert.equal(ordem().restantePresumido,0);assert.equal(relatorio().pagamentos.length,1);
 ...
 assert.equal(vale().financeiro.recebido,0);assert.equal(vale().financeiro.presumido,0);assert.equal(relatorio().pagamentos.length,0);
