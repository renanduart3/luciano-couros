...
import { distribuirCentavos, sugerirValores } from '../lib/distribuicaoPagamento';
...
export function LinhaPagamento({ children, colunasAntes = 0, pagamento, clienteId, clienteNome, clienteDocumento, saldo,
  ...
  useEffect(() => { setCredito(ls => sugerirValores(ls, parseBrazilianNumber(valor))); }, [valor]);
  ...
  const total = titulo ? Math.round(titulos.reduce((s, t) => s + (t.status === "recusado" ? 0 : Number(t.valor || 0)), 0) * 100) / 100 : forma === 'cartao_credito' && credito.length ? Math.round(credito.reduce((s, l) => s + l.valor, 0) * 100) / 100 : parseBrazilianNumber(valor);
  const colunas = colunasAntes + 5;
  const iniciar = (criar = false) => {
    const existente = criar ? undefined : pagamento;
  ...
  const salvar = async () => {
    ...
    if (!novo && pin.length < 4) return setErro("Informe a senha do gerente.");
    if (situacao === "em_aberto" && (!plano || !confirmado)) return setErro("Confira e confirme o estorno abaixo.");
  ...
  return <>
    ...
    {editando && <tr className="payment-compact bg-blue-50"><td colSpan={colunas} className="px-3 pb-3">
      ...
      {!novo && <label className="mt-2 inline-flex items-center gap-2 text-[10px] font-bold">Senha do gerente<input type="password" autoComplete="off" maxLength={64} disabled={saving || concluido} value={pin} onChange={e => setPin(e.target.value)} className={`${campo} max-w-44`}/></label>}
