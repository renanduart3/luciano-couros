    ...
    <section className="receipt-copy budget-copy">
      ...
      <header className="receipt-header">
        ...
        <div className="receipt-document-meta">
          ...
          <span>Data: {formatDate(orcamento.data)}</span>
          <span>Nº {String(orcamento.numeroSequencial).padStart(6, "0")}</span>
        </div>
      </header>
      ...
      <footer className="receipt-footer">
        ...
        <div className="receipt-total"><span>TOTAL R$</span><strong>{formatCurrency(orcamento.totalLiquido)}</strong></div>
        <div className="receipt-due"><span>VALIDADE:</span><strong>{orcamento.validade ? formatDate(orcamento.validade) : "—"}</strong></div>
      </footer>
    </section>
  );
}
