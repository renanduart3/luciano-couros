    ...
    <div role="dialog" aria-modal="true" className="flex max-h-[90vh] w-full flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
      ...
      <div className="space-y-4 overflow-y-auto bg-slate-100 p-4">
        ...
        {abaDetalhe === "parcelas" ? <div className="overflow-hidden rounded-xl border border-slate-300 bg-white">
          <div className="border-b border-slate-300 bg-slate-50 p-3"><p className="text-xs font-black uppercase text-slate-700">Parcelamento e pagamentos</p><p className="mt-1 text-sm font-black text-slate-950">{ordem.clienteNome}</p><p className="text-[10px] font-bold text-slate-500">CPF/CNPJ: {ordem.clienteDocumento || "NÃO INFORMADO"}</p></div>
          <div className="overflow-x-auto"><table className="w-full min-w-[1040px] text-xs"><thead className="bg-slate-900 text-[10px] font-black uppercase text-white"><tr><th className="p-2 text-left">Parcela</th><th className="p-2 text-left">Vencimento</th><th className="p-2 text-right">Previsto</th><th className="p-2 text-right">Pago</th><th className="p-2 text-left">Data do pagamento</th><th className="p-2 text-right">Pagamento</th><th className="w-[210px] p-2 text-center">Ações</th></tr></thead><tbody className="divide-y divide-slate-200">{ordem.parcelas.map((parcela) => <tr key={parcela.id} className={parcela.status === "paga" ? "bg-emerald-50/60" : parcelaEmPagamento === parcela.id ? "bg-sky-50" : "bg-white"}><td className="p-2 font-black">{parcela.numero}/{ordem.parcelas.length}</td><td className="p-2 font-bold">{formatDate(parcela.vencimento)}</td><td className="p-2 text-right font-mono font-bold">{formatCurrency(parcela.valor)}</td><td className="p-2 text-right font-mono font-bold text-emerald-800">{formatCurrency(parcela.valorPago)}</td>{parcela.status === "pendente" && ordem.status === "aberta" ? <><td className="p-2"><input type="date" readOnly={pagamentoTitulo} value={datasPagamento[parcela.id] || hojeIso()} onFocus={() => setParcelaEmPagamento(parcela.id)} onChange={(event) => setDatasPagamento((atuais) => ({ ...atuais, [parcela.id]: event.target.value }))} className={`min-h-9 rounded-lg border px-2 font-bold ${pagamentoTitulo ? "cursor-not-allowed border-slate-200 bg-slate-100 text-slate-500" : "border-slate-300"}`}/></td><td className="p-2 text-right"><input type="text" readOnly={pagamentoTitulo} inputMode="decimal" value={pagamentoTitulo ? totalTitulos.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : valoresPagamento[parcela.id] || ""} onFocus={() => setParcelaEmPagamento(parcela.id)} onChange={(event) => setValoresPagamento((atuais) => ({ ...atuais, [parcela.id]: event.target.value }))} className={`min-h-9 w-32 rounded-lg border px-2 text-right font-mono font-black ${pagamentoTitulo ? "cursor-not-allowed border-slate-200 bg-slate-100 text-slate-600" : "border-emerald-300 bg-emerald-50 text-emerald-900"}`}/><ResumoParcelamentoCartao formaPagamento={formaPagamento} parcelasCartao={parcelasCartao} valorTotal={pagamentoTitulo ? totalTitulos : parseBrazilianNumber(valoresPagamento[parcela.id] || "")} className="mt-1 w-48 text-left font-sans" /></td><td className="p-2"><div className="grid grid-cols-2 justify-center gap-2"><button type="button" disabled={saving} onClick={() => void registrarPagamento(parcela.id)} className="inline-flex min-h-9 w-24 items-center justify-center gap-1 rounded-lg bg-emerald-700 text-[10px] font-black uppercase text-white disabled:opacity-40"><Coins size={14}/> Registrar</button><button type="button" disabled={saving || !parcela.ultimoRecebimentoId} onClick={() => { if (parcela.ultimoRecebimentoId) setEditandoRecebimentoId(parcela.ultimoRecebimentoId); }} className="inline-flex min-h-9 w-24 items-center justify-center gap-1 rounded-lg border border-amber-400 bg-amber-50 text-[10px] font-black uppercase text-amber-900 disabled:cursor-not-allowed disabled:opacity-35"><Edit3 size={13}/> Editar</button></div></td></> : <><td className="p-2 font-bold text-slate-700">{parcela.dataPagamento ? formatDate(parcela.dataPagamento) : "—"}</td><td className="p-2 text-center"><span className={`rounded-lg px-2 py-1 text-[10px] font-black ${parcela.status === "paga" ? "bg-emerald-100 text-emerald-800" : "bg-slate-200 text-slate-700"}`}>{parcela.status === "paga" ? "QUITADA" : parcela.status.toUpperCase()}</span></td><td className="p-2"><div className="grid grid-cols-2 justify-center gap-2"><span className="w-24" aria-hidden="true"/>{parcela.ultimoRecebimentoId ? <button type="button" disabled={saving} onClick={() => setEditandoRecebimentoId(parcela.ultimoRecebimentoId!)} className="inline-flex min-h-9 w-24 items-center justify-center gap-1 rounded-lg border border-amber-400 bg-amber-50 text-[10px] font-black uppercase text-amber-900 disabled:opacity-40"><Edit3 size={13}/> Editar</button> : <span className="w-24" aria-hidden="true"/>}</div></td></>}</tr>)}</tbody></table></div>
          ...
          <div className="divide-y divide-slate-200">{(ordem.eventos || []).map((evento) => <div key={evento.id} className="flex items-center gap-3 p-3 text-sm"><span className="shrink-0 font-mono text-xs font-black text-slate-500">{formatDate(evento.data)}</span><p className={`min-w-0 flex-1 ${evento.tipo === "estorno" ? "font-bold text-red-700" : "font-bold text-slate-800"}`}>{evento.texto}{evento.formaPagamento && evento.tipo === "pagamento" ? ` Forma: ${evento.formaPagamento.replaceAll("_", " ").toUpperCase()}.` : ""}</p><ResumoParcelamentoCartao formaPagamento={evento.formaPagamento || ""} parcelasCartao={evento.parcelasCartao} valorTotal={Number(evento.valor || 0)} className="shrink-0" />{evento.tipo === "pagamento" && evento.recebimentoId && <button type="button" onClick={() => setEditandoRecebimentoId(evento.recebimentoId!)} className="inline-flex min-h-8 shrink-0 items-center gap-1 rounded-lg border border-slate-300 bg-white px-2 text-[10px] font-black uppercase"><Edit3 size={13}/>Editar</button>}</div>)}</div>
        </div>}
