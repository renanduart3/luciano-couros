...
function ResumoCompartilhavelOrdem({ ordem, onEditarVales }: { ordem: OrdemCobranca; onEditarVales: () => void }) {
  const eventosPagamentoEstornados = new Set((ordem.eventos || []).filter((evento) => evento.tipo === "estorno").map((evento) => evento.id.replace(/_estorno$/, "")));
  const pagamentosPorParcela = new Map<number, OrdemCobranca["eventos"]>();
  for (const evento of ordem.eventos || []) {
    if (evento.tipo !== "pagamento" || !evento.parcelaNumero || eventosPagamentoEstornados.has(evento.id)) continue;
    pagamentosPorParcela.set(evento.parcelaNumero, [...(pagamentosPorParcela.get(evento.parcelaNumero) || []), evento]);
  }
  ...
  return <section aria-label="Resumo da ordem para compartilhamento" className="overflow-hidden rounded-2xl border-2 border-slate-400 bg-white shadow-sm">
    ...
    <div className="grid xl:grid-cols-[0.85fr_1.35fr]">
      ...
      <div>
        <div className="flex min-h-11 items-center justify-between border-b border-slate-200 bg-slate-50 px-3"><div><p className="text-[11px] font-black uppercase text-slate-800">Parcelas e pagamentos</p><p className="text-[9px] font-bold text-slate-500">Resumo atualizado do controle abaixo</p></div><span className={`rounded-md px-2 py-1 text-[9px] font-black ${statusClass[ordem.status]}`}>{statusLabel[ordem.status]}</span></div>
        <div className="divide-y divide-slate-100">{ordem.parcelas.map((parcela) => {
          const pagamentos = pagamentosPorParcela.get(parcela.numero) || [];
          const situacao = parcela.status === "paga" ? "QUITADA" : parcela.status === "pendente" ? "EM ABERTO" : parcela.status.toUpperCase();
          return <div key={parcela.id} className="grid gap-1 px-3 py-2 text-[10px] sm:grid-cols-[86px_90px_1fr_auto] sm:items-center"><strong className="text-xs">Parcela {parcela.numero}/{ordem.parcelas.length}</strong><span className="font-bold text-slate-600">{formatDate(parcela.vencimento)}</span><div className="flex flex-wrap gap-x-3 gap-y-0.5 font-mono"><span>Previsto <strong>{formatCurrency(parcela.valor)}</strong></span><span className="text-emerald-800">Pago <strong>{formatCurrency(parcela.valorPago)}</strong></span><span className="text-amber-900">Saldo <strong>{formatCurrency(parcela.saldo)}</strong></span>{pagamentos.length > 0 && <span className="basis-full font-sans text-[9px] font-bold text-slate-500">{pagamentos.map((evento) => `${formatDate(evento.data)} · ${formatCurrency(Number(evento.valor || 0))} · ${String(evento.formaPagamento || "").replaceAll("_", " ").toUpperCase()}`).join(" | ")}</span>}</div><span className={`justify-self-start rounded-md px-2 py-1 text-[9px] font-black sm:justify-self-end ${parcela.status === "paga" ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-900"}`}>{situacao}</span></div>;
        })}</div>
      </div>
    </div>
  </section>;
