...
export function RecebimentoDetalhesModal({ recebimentoId, onSaved, onClose, onComprovante, somenteLeitura = false }: {
  ...
  const vinculadas = ordens.filter(o => o.id === pagamento?.ordemCobrancaId || o.pagamentos?.some(p => p.id === recebimentoId));
  ...
  if (ordem) return <OrdemCobrancaDetalhesModal ordem={ordem} onClose={() => setOrdem(null)} onChanged={atualizada => { setOrdem(atualizada); void onSaved(); }} />;
    ...
    <div className="space-y-2 p-3">
      ...
      {pagamento && <>
        <div className="flex flex-wrap gap-2">
          {vinculadas.map(o => <button key={o.id} type="button" disabled={editando || saving} onClick={() => setOrdem(o)} className="rounded border border-blue-300 px-2 py-1 font-bold text-blue-700">Abrir ordem #{o.numeroSequencial}</button>)}
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[700px] text-left text-xs">
            ...
            <tbody><LinhaPagamento pagamento={pagamento} clienteId={pagamento.clienteId} clienteNome={pagamento.clienteNome}
              ...
              ordemCobrancaId={pagamento.ordemCobrancaId || undefined} editavel={gerente && !somenteLeitura} onEditingChange={setEditando} onSavingChange={setSaving} onSaved={atualizar}/></tbody>
          </table>
        </div>
