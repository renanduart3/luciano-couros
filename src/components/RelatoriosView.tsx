...
export function RelatoriosView() {
  ...
  const exportarCsv = () => {
    ...
    let csv = "\uFEFF";
    if (aba === "vendas") {
      csv += "CLIENTE;VALOR TOTAL;M;R$ METRO;UN;R$ UN;CUSTO;LUCRO;DATA\n";
      linhasVendas.forEach((item: any) => { csv += `${csvCelula(item.clienteNome)};${item.totalLiquido};${item.metragem};${item.valorMetros};${item.unidades};${item.valorUnidades};${item.custo};${item.lucro};${item.data}\n`; });
...
function TabelaVendas({ linhas }: { linhas: any[] }) {
  return <div className="overflow-x-auto"><table className="w-full table-fixed text-[11px] xl:text-xs">
    <colgroup><col className="w-[18%]" /><col className="w-[12%]" /><col className="w-[7%]" /><col className="w-[11%]" /><col className="w-[7%]" /><col className="w-[11%]" /><col className="w-[11%]" /><col className="w-[11%]" /><col className="w-[12%]" /></colgroup>
