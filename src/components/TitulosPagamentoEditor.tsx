...
export function TitulosPagamentoEditor({ formaPagamento, clienteId, clienteNome, clienteDocumento, valorPagamento, titulos, onChange, referenciaPagamento, limiteLinhas = 5 }: Props) {
  ...
  return <section className="rounded-lg border border-sky-300 bg-sky-50 p-2">
    <div className="mb-1.5 flex flex-wrap items-end justify-between gap-2">
      <div><div className="flex flex-wrap items-center gap-2"><h3 className="text-xs font-black uppercase text-sky-950">{boleto ? "Duplicatas (boletos)" : "Cheques"}</h3>{referenciaPagamento && <span className="rounded-md border border-blue-300 bg-blue-700 px-2 py-1 text-[10px] font-black uppercase text-white">{referenciaPagamento}</span>}</div><p className="text-[11px] font-semibold text-sky-800">Informe uma linha para cada {boleto ? "boleto" : "cheque"} recebido. Máximo de {limiteLinhas} por pagamento.</p></div>
    ...
    <div className="space-y-1.5">
      {titulos.map((titulo, indice) => <div key={titulo.id || `${formaPagamento}-${indice}`} className="rounded-md border border-sky-200 bg-white p-1.5">
        <div className="grid grid-cols-2 items-end gap-1.5 lg:grid-cols-[78px_190px_145px_88px_124px_135px_minmax(160px,1fr)_26px]">
          <span title={referenciaPagamento || undefined} className={`mb-0.5 inline-flex h-7 items-center justify-center rounded border px-1 text-center text-[9px] font-black ${titulo.status === "recusado" ? "border-red-200 bg-red-50 text-red-700" : "border-sky-200 bg-sky-50 text-sky-800"}`}>{titulo.status === "recusado" ? "RECUSADO" : referenciaPagamento || `TÍTULO ${indice + 1}`}</span>
          ...
          <label className="text-[8px] font-black uppercase text-slate-600">Observação <span className="font-bold text-slate-400">{titulo.status === "recusado" ? "(recusado)" : referenciaPagamento ? `(${referenciaPagamento.toLowerCase()})` : "(opcional)"}</span><input disabled={titulo.status === "recusado"} value={titulo.observacao || ""} onChange={(e) => atualizar(indice, { observacao: e.target.value.slice(0, 300) })} placeholder={referenciaPagamento || "Observação"} className="titulo-pagamento-input mt-0.5 h-7 w-full rounded border border-slate-200 bg-slate-50 px-1.5 text-[11px] font-bold normal-case disabled:bg-red-50 disabled:text-red-800" /></label>
