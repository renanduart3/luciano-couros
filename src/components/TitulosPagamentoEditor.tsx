  ...
  return <section className="rounded-lg border border-sky-300 bg-sky-50 p-2">
    ...
    <div className="space-y-1.5">
      {titulos.map((titulo, indice) => <div key={titulo.id || `${formaPagamento}-${indice}`} className="rounded-md border border-sky-200 bg-white p-1.5">
        <div className="grid grid-cols-2 items-end gap-1.5 lg:grid-cols-[20px_200px_155px_88px_124px_145px_minmax(180px,1fr)_26px]">
          ...
          <label className="text-[8px] font-black uppercase text-slate-600">Observação <span className="font-bold text-slate-400">{titulo.status === "recusado" ? "(recusado)" : "(opcional)"}</span><input disabled={titulo.status === "recusado"} value={titulo.observacao || ""} onChange={(e) => atualizar(indice, { observacao: e.target.value.slice(0, 300) })} placeholder="Observação" className="titulo-pagamento-input mt-0.5 h-7 w-full rounded border border-slate-200 bg-slate-50 px-1.5 text-[11px] font-bold normal-case disabled:bg-red-50 disabled:text-red-800" /></label>
