...
const estaEmAberto = (vale: Venda) => vale.status !== "cancelada" && financeiroVale(vale).restante > 0.005;
...
export function ValesView({ onRefreshStats }: ValesViewProps) {
  ...
  const valesFiltrados = useMemo(() => vales.filter((vale) => {
    ...
    if (status === "quitados" && (vale.status === "cancelada" || financeiroVale(vale).restante > 0.005)) return false;
  ...
  const valesOrdenados = useMemo(() => [...valesFiltrados].sort((a, b) => {
    ...
    if (ordenacao === "valor_desc") return financeiroVale(b).restante - financeiroVale(a).restante || Number(b.numeroSequencial) - Number(a.numeroSequencial);
    if (ordenacao === "valor_asc") return financeiroVale(a).restante - financeiroVale(b).restante || Number(b.numeroSequencial) - Number(a.numeroSequencial);
  ...
  const totais = useMemo(() => {
    const abertos = valesFiltrados.filter(v => estaEmAberto(v) && Number(v.saldoRestante) > 0.005 && !ordemAtivaPorVale.has(v.id));
    ...
    return {
      ...
      restante: valesFiltrados.filter(v=>v.status!=="cancelada").reduce((s,v)=>s+financeiroVale(v).restante,0),
              ...
              <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                ...
                <div className="divide-y divide-slate-100 md:hidden">
                  {valesPagina.map((vale) => {
                    const atraso = diasEmAtraso(vale.vencimento);
                    const ordemAtiva = ordemAtivaPorVale.get(vale.id);
                    return <article key={vale.id} className={`space-y-2 p-3 ${selecionados.has(vale.id) ? "bg-emerald-50" : ""}`}><div className="flex items-start gap-3">{clienteId && <input aria-label={`Selecionar vale ${vale.numeroSequencial}`} type="checkbox" checked={selecionados.has(vale.id)} disabled={!estaEmAberto(vale) || Number(vale.saldoRestante) <= 0.005 || ordemAtivaPorVale.has(vale.id)} onChange={() => alternarVale(vale)} className="mt-1 h-6 w-6 shrink-0 accent-emerald-700"/>}<div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-extrabold text-slate-400">VALE #{vale.numeroSequencial}</p>{ordemAtiva && <button type="button" onClick={() => setOrdemDetalhada(ordemAtiva)} className="mt-1 rounded-md bg-blue-100 px-2 py-1 text-[10px] font-black text-blue-800">ORDEM #{ordemAtiva.numeroSequencial}</button>}<h2 className="mt-1 text-base font-black uppercase text-slate-950">{vale.clienteNome || "Cliente não informado"}</h2></div><div className="text-right"><p className="text-[9px] font-black uppercase text-slate-400">Valor</p><p className="font-mono text-lg font-black text-slate-950">{formatCurrency(vale.totalLiquido)}</p><p className="text-xs">Recebido: {formatCurrency(financeiroVale(vale).recebido)}</p><p className="text-xs">Restante: {formatCurrency(financeiroVale(vale).restante)}</p></div></div><div className="mt-1 flex flex-wrap items-center gap-2 text-xs font-bold text-slate-500"><span className="inline-flex items-center gap-1"><CalendarClock size={14} /> Emissão {formatDate(vale.data)}</span><span>Último pagamento: {vale.ultimoPagamentoData ? formatDate(vale.ultimoPagamentoData) : "—"}</span>{vale.status === "cancelada" ? <span className="rounded-lg bg-slate-200 px-2 py-1 text-slate-700">Cancelado</span> : financeiroVale(vale).restante <= 0.005 ? <span className="rounded-lg bg-emerald-100 px-2 py-1 text-emerald-700">Quitado</span> : atraso > 0 ? <span className="rounded-lg bg-red-100 px-2 py-1 text-red-700">{atraso} dias em atraso</span> : <span className="rounded-lg bg-amber-100 px-2 py-1 text-amber-700">A vencer</span>}</div></div></div><button type="button" onClick={() => void abrirDetalhesVale(vale)} className="inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-lg bg-slate-900 px-3 text-xs font-black uppercase text-white"><Eye size={15} /> Ver itens e comprovante</button></article>;
                  })}
                </div>
