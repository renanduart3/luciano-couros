...
export function ValesView({ onRefreshStats }: ValesViewProps) {
  ...
  const totais = useMemo(() => {
    ...
    return {
      ...
      pago: valesFiltrados.filter(v=>v.status!=="cancelada").reduce((s,v)=>s+financeiroVale(v).presumido,0),
      ...
      {tab === "ordens" ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            {[["Devedor",totais.negociado],["Pago",totais.pago],["Restante",totais.restante]].map(([label,valor]) => <div key={label} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><p className="text-xs font-bold text-slate-500">{label}</p><p className="mt-2 text-2xl font-black">{formatCurrency(Number(valor))}</p></div>)}
          </div>
