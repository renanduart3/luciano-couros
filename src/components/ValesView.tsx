...
const estaEmAberto = (vale: Venda) => vale.status !== "cancelada" && financeiroVale(vale).restantePresumido > 0.005;
...
export function ValesView({ onRefreshStats }: ValesViewProps) {
  ...
  const valesFiltrados = useMemo(() => vales.filter((vale) => {
    ...
    if (status === "quitados" && (vale.status === "cancelada" || financeiroVale(vale).restantePresumido > 0.005)) return false;
  ...
  const valesOrdenados = useMemo(() => [...valesFiltrados].sort((a, b) => {
    ...
    if (ordenacao === "valor_desc") return financeiroVale(b).restantePresumido - financeiroVale(a).restantePresumido || Number(b.numeroSequencial) - Number(a.numeroSequencial);
    if (ordenacao === "valor_asc") return financeiroVale(a).restantePresumido - financeiroVale(b).restantePresumido || Number(b.numeroSequencial) - Number(a.numeroSequencial);
  ...
  const valesSelecionados = useMemo(() => valesFiltrados.filter((vale) => selecionados.has(vale.id) && !ordemAtivaPorVale.has(vale.id)), [valesFiltrados, selecionados, ordemAtivaPorVale]);
  ...
  const totais = useMemo(() => {
    const abertos = valesFiltrados.filter(v => estaEmAberto(v) && !ordemAtivaPorVale.has(v.id));
    const vencidos = abertos.filter((vale) => diasEmAtraso(vale.vencimento) > 0);
    return {
      negociado: valesFiltrados.filter(v=>v.status!=="cancelada").reduce((s,v)=>s+financeiroVale(v).negociado,0),
      recebido: valesFiltrados.filter(v=>v.status!=="cancelada").reduce((s,v)=>s+financeiroVale(v).recebido,0),
      restante: valesFiltrados.filter(v=>v.status!=="cancelada").reduce((s,v)=>s+financeiroVale(v).restantePresumido,0),
      aberto: abertos.reduce((total, vale) => total + financeiroVale(vale).restantePresumido, 0),
      vencido: vencidos.reduce((total, vale) => total + financeiroVale(vale).restantePresumido, 0),
      ...
      {tab === "ordens" ? (
        ...
        <>
          ...
          {clienteId && <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-emerald-300 bg-emerald-50 p-2.5">
            <div><p className="text-xs font-black uppercase text-emerald-950">Selecione os vales em aberto para cobrar</p><p className="text-xs font-bold text-emerald-700">{valesSelecionados.length} selecionado(s) • {formatCurrency(valesSelecionados.reduce((total, vale) => total + financeiroVale(vale).restantePresumido, 0))}</p></div>
            <div className="flex flex-wrap gap-2"><button type="button" onClick={() => setSelecionados(new Set(valesFiltrados.filter(v => estaEmAberto(v) && Number(v.saldoRestante) > 0.005 && !ordemAtivaPorVale.has(v.id)).map((vale) => vale.id)))} className="inline-flex items-center gap-2 rounded-lg border border-emerald-400 bg-white px-3 py-2 text-xs font-black uppercase text-emerald-900"><CheckSquare2 size={16}/> Selecionar abertos</button><button type="button" disabled={valesSelecionados.length !== 1} title="Para pagar vários vales, gere uma ordem de cobrança" onClick={() => setPagamentoAberto(true)} className="inline-flex items-center gap-2 rounded-lg bg-slate-900 px-4 py-2 text-xs font-black uppercase text-white disabled:cursor-not-allowed disabled:opacity-40"><Coins size={16}/> Registrar pagamento</button><button type="button" disabled={valesSelecionados.length === 0} onClick={() => setCobrancaAberta(true)} className="inline-flex items-center gap-2 rounded-lg bg-emerald-700 px-4 py-2 text-xs font-black uppercase text-white disabled:cursor-not-allowed disabled:opacity-40"><MessageCircle size={16}/> Gerar ordem</button></div>
          </div>}

          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            {[['Valor',totais.negociado],['Recebido',totais.recebido],['Restante',totais.restante]].map(([label,valor]) => <div key={label} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><p className="text-xs font-bold text-slate-500">{label}</p><p className="mt-2 text-2xl font-black">{formatCurrency(Number(valor))}</p></div>)}
          </div>
              ...
              <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                  ...
                  <table className="w-full min-w-[900px] text-left text-sm">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {valesPagina.map((vale) => {
                        ...
                        return <tr key={vale.id} className={selecionados.has(vale.id) ? "bg-emerald-50" : "hover:bg-slate-50"}>{clienteId && <td className="p-2 text-center"><input aria-label={`Selecionar vale ${vale.numeroSequencial}`} type="checkbox" checked={selecionados.has(vale.id)} disabled={!estaEmAberto(vale) || Number(vale.saldoRestante) <= 0.005 || ordemAtivaPorVale.has(vale.id)} onChange={() => alternarVale(vale)} className="h-5 w-5 accent-emerald-700"/></td>}<td className="p-2"><span className="font-mono font-extrabold text-slate-700">#{vale.numeroSequencial}</span>{ordemAtiva && <button type="button" onClick={() => setOrdemDetalhada(ordemAtiva)} className="ml-2 rounded-md bg-blue-100 px-2 py-1 text-[10px] font-black text-blue-800 hover:bg-blue-200">ORDEM #{ordemAtiva.numeroSequencial}</button>}</td><td className="p-2 font-extrabold text-slate-950">{vale.clienteNome || "Cliente não informado"}</td><td className="p-2 font-bold text-slate-700">{formatDate(vale.data)}</td><td className="p-2">{vale.status === "cancelada" ? <span className="rounded-lg bg-slate-200 px-2 py-1 text-xs font-extrabold text-slate-700">Cancelado</span> : posicao.restantePresumido <= 0.005 ? <span className="rounded-lg bg-emerald-100 px-2 py-1 text-xs font-extrabold text-emerald-700">Coberto</span> : atraso > 0 ? <span className="rounded-lg bg-red-100 px-2 py-1 text-xs font-extrabold text-red-700">{atraso} dias em atraso</span> : <span className="rounded-lg bg-amber-100 px-2 py-1 text-xs font-extrabold text-amber-700">A vencer</span>}</td><td className="p-2 text-right font-mono font-bold">{formatCurrency(vale.totalLiquido)}</td><td className="p-2 text-right font-mono text-blue-800">{formatCurrency(posicao.recebido)}</td><td className="p-2 text-right font-mono text-emerald-800">{formatCurrency(posicao.presumido)}</td><td className={`p-2 text-right font-mono ${posicao.excedentePresumido > 0.005 ? "text-violet-800" : "text-amber-800"}`}>{formatCurrency(posicao.excedentePresumido || posicao.restantePresumido)}</td><td className="p-2 font-bold text-slate-700">{vale.ultimoPagamentoData ? formatDate(vale.ultimoPagamentoData) : "—"}</td><td className="p-2 text-center"><button type="button" onClick={() => void abrirDetalhesVale(vale)} className="inline-flex min-h-9 items-center gap-2 rounded-lg bg-slate-900 px-3 text-xs font-black uppercase text-white"><Eye size={15} /> Detalhes</button></td></tr>;
                      })}
                    </tbody>
                  </table>
                </div>
                ...
                <div className="divide-y divide-slate-100 md:hidden">
                  {valesPagina.map((vale) => {
                    ...
                    const ordemAtiva = ordemAtivaPorVale.get(vale.id);
                    const posicao = financeiroVale(vale);
                    return <article key={vale.id} className={`space-y-2 p-3 ${selecionados.has(vale.id) ? "bg-emerald-50" : ""}`}><div className="flex items-start gap-3">{clienteId && <input aria-label={`Selecionar vale ${vale.numeroSequencial}`} type="checkbox" checked={selecionados.has(vale.id)} disabled={!estaEmAberto(vale) || Number(vale.saldoRestante) <= 0.005 || ordemAtivaPorVale.has(vale.id)} onChange={() => alternarVale(vale)} className="mt-1 h-6 w-6 shrink-0 accent-emerald-700"/>}<div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-extrabold text-slate-400">VALE #{vale.numeroSequencial}</p>{ordemAtiva && <button type="button" onClick={() => setOrdemDetalhada(ordemAtiva)} className="mt-1 rounded-md bg-blue-100 px-2 py-1 text-[10px] font-black text-blue-800">ORDEM #{ordemAtiva.numeroSequencial}</button>}<h2 className="mt-1 text-base font-black uppercase text-slate-950">{vale.clienteNome || "Cliente não informado"}</h2></div><div className="text-right"><p className="text-[9px] font-black uppercase text-slate-400">Valor</p><p className="font-mono text-lg font-black text-slate-950">{formatCurrency(vale.totalLiquido)}</p><p className="text-xs">Entrou: {formatCurrency(posicao.recebido)}</p><p className="text-xs text-emerald-800">Presumido: {formatCurrency(posicao.presumido)}</p><p className="text-xs">{posicao.excedentePresumido > 0.005 ? "Excedente" : "Devido"}: {formatCurrency(posicao.excedentePresumido || posicao.restantePresumido)}</p></div></div><div className="mt-1 flex flex-wrap items-center gap-2 text-xs font-bold text-slate-500"><span className="inline-flex items-center gap-1"><CalendarClock size={14} /> Emissão {formatDate(vale.data)}</span><span>Último pagamento: {vale.ultimoPagamentoData ? formatDate(vale.ultimoPagamentoData) : "—"}</span>{vale.status === "cancelada" ? <span className="rounded-lg bg-slate-200 px-2 py-1 text-slate-700">Cancelado</span> : posicao.restantePresumido <= 0.005 ? <span className="rounded-lg bg-emerald-100 px-2 py-1 text-emerald-700">Coberto</span> : atraso > 0 ? <span className="rounded-lg bg-red-100 px-2 py-1 text-red-700">{atraso} dias em atraso</span> : <span className="rounded-lg bg-amber-100 px-2 py-1 text-amber-700">A vencer</span>}</div></div></div><button type="button" onClick={() => void abrirDetalhesVale(vale)} className="inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-lg bg-slate-900 px-3 text-xs font-black uppercase text-white"><Eye size={15} /> Ver itens e comprovante</button></article>;
                  })}
                </div>
